...
describe('routes/instances +page', () => {
	it('renders instances parsed from TOML and shows defaults for missing details', async () => {
		// Provide mock data that matches what the +page.ts load function returns
		const mockData = {
			instances: [
				{
					name: 'Alpha',
					...
					details: {
						version: '1.2.3',
						last_check: 1700000000000 as EpochTimeStamp,
						uptime: 99
					}
				},
				{
					...
					details: {
						version: 'N/A',
						last_check: 0 as EpochTimeStamp,
						uptime: 0
					}
				}
			]
		...
		const { container } = render(InstancesPage, { props: { data: mockData } });
