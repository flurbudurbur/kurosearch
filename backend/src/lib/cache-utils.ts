 ...
 */
export async function invalidateCache(key: string, notifyClients = false): Promise<boolean> {
	const client = getValkeyClient();
 ...
 */
export async function invalidateCachePattern(pattern: string, notifyClients = false): Promise<number> {
	const client = getValkeyClient();
